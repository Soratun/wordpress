...
class Registration {
    ...
    public function get_action_links( $args = [] ) {
        ...
        if ( $args['register'] && get_option( 'users_can_register' ) ) {
            $links[] = sprintf(
                '<a href="%s">%s</a>', $this->get_action_url( 'register' ), esc_html__( 'Register', 'wp-user-frontend' )
            );
        }
    ...
    public function process_registration() {
        if ( ! empty( $_POST['wpuf_registration'] ) && ! empty( $_POST['_wpnonce'] ) ) {
            ...
            $validation_error = apply_filters(
                'wpuf_process_registration_errors', $validation_error, $reg_fname, $reg_lname, $reg_email, $log, $pwd1,
                $pwd2
            );
    ...
    public function registration_page_redirects() {
        ...
        if ( $this->is_activated_subscription_on_registration() ) {
            ...
            if ( empty( $subscription_page_id ) ) {
                WP_User_Frontend::log(
                    'subscription-on-registration',
                    esc_html__( 'Subscription Page settings not set in admin settings', 'wp-user-frontend' )
                );
    ...
    public function show_errors() {
        ...
        if ( $this->registration_errors ) {
            foreach ( $this->registration_errors as $error ) {
                printf(
                    '<div class="wpuf-error">%s</div>', wp_kses( $error, $allowed_html )
                );
            }
        }
    }
